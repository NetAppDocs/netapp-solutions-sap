...
When SAP HANA persistence encryption is enable and depending on the used secure store for encryption root keys, instance SFSS or local secure store LSS, additional steps are required before you can recover the SAP HANA database at the target system.

With SFSS root keys are stored within the file system and must be backed up separetely in addtion to the HANA database. The root key backups must then be imported at the target system to enable the recovery of the HANA database. This has to be done only once, If the root keys are not changed. 

With LSS the root keys are backed up and restored together with the HANA database in a single operation. In order to be able to recover the HANA database, you need to have the password that has been used for the backup of the root keys. 

The required operations are described in more details in the following chapters.
...
NOTE: You need to use at least HANA version 2.00.088, otherwise described procedure will not work, see also SAP note 3613110.

In order to recover a HANA system which uses encyrption and LSS, you need the password, that has been used to backup the root keys.

The SQL statement
.... 
alter system set encryption root keys backup password <backup-pw-of-source-system>;
....
can be used to set the password of the source system in the LSS configuration of the target system. The same statement must be used for the system and tenant database.

If the password has not been set upfront, you can provide the password during the recovery operation.
