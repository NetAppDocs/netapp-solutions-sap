...
The SAP host agent was installed on this host as well. The NetApp provider script as well as the Ansible playbooks were configured on the Ansible control node as described in the link:lama-ansible-appendixs.html[“Appendix: Provider Script Configuration.”]
