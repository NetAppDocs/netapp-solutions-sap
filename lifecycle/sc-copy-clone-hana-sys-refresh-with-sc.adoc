...
| |SAP HANA persistence encryption |SFSS: Initially, or if root keys have been changed at the source system, root key backup(s) must be imported at the target system before recovery can be executed.
LSS: initially, or if root key backup password has been changed, the password must be set in the LSS configutation.
...
| |SAP HANA persistence encryption |SFSS: If a DBID of the source system does not exist before at the target system, the system database must be recovered first, before the root key backup of this tenant can be imported.
LSS: LSS topology will automatically be synced with HANA topology during tenant database(s) recovery.
...
as described in link:../backup/hana-sc-generic-scope.html[SAP HANA Backup and Recovery with SnapCenter]
...
In a Fibre Channel SAN setup, you need to provide the used protocol in the next screen.
[NOTE]
