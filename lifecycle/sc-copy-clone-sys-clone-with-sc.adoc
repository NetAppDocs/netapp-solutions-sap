...
[NOTE]
========
The SnapCenter clone delete workflow does not delete the `fstab` entry by default.

In order for the `fstab` entry to be deleted automatically during the clone deletion process, the clone must first be recovered and made an auto-discovered resource. If this step is not performed, the `fstab` entry is not automatically deleted and needs to be removed manually.
========
