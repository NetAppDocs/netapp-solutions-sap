...
In addition to the tasks described in the section link:hana-aff-nfs-host-setup.adoc.html["Host Setup,"] the additional mount points and `fstab` entries for the new additional data volume/s must be created and the new volumes must be mounted.
...
image:saphana_aff_nfs_image18.jpg["Figure showing input/output dialog or representing written content"]
