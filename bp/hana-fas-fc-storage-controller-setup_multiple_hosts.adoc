...
The `/hana/shared` volume must be accessible by all HANA hosts and is therefore exported by using NFS. Even though there are no specific performance KPIs for the `/hana/shared` a file system, NetApp recommends using a 10Gb Ethernet connection.
...
[NOTE]
If SAP Local Secure Store service (LSS) is used, the `/lss/shared/SID` directory must be accessible at all hosts. NetApp recommends creating a volume for the `/lss/shared/SID` directory and mount it at all hosts using NFS.
