...
[NOTE]
Please note that disabling numering ids requires user management as described in link:hana-fas-nfs-install-prep-nfsv4.adoc.html[SAP HANA installation preparations for NFSv4.]

[NOTE]
The NFSv4 domain ID must be set to the same value on all Linux servers (/`etc/idmapd.conf`) and SVMs, as described in link:hana-fas-nfs-install-prep-nfsv4.adoc.html[SAP HANA installation preparations for NFSv4.]
