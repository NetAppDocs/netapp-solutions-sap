...
The following table shows the LUN configuration of a single-host SAP HANA system.
...
lun create -path FC5-lss_shared -size 60g -ostype linux  -class regular 
....
[NOTE]
...
lun map -path FC5-lss_shared -igroup HANA-FC5
....
...
All required LUNs for a SAP HANA single-host system have been created. May create an additional FC-lss_shared if Local Secure Store Service (LSS) is used.
...
image:saphana_asa_fc_image19_5.png["ONTAP System Manager - LUN Overview"]