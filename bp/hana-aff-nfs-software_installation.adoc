---
sidebar: sidebar
permalink: bp/hana-aff-nfs-software_installation.adoc.html
...
. Using the SAP `hdbclm` installation tool, start the installation by running the following command at one of the worker hosts. Use the `addhosts` option to add the second worker (`sapcc-hana-tst-03`) and the standby host (`sapcc-hana-tst-04`).
+
....
