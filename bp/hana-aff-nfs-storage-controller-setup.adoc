---
sidebar: sidebar
permalink: bp/hana-aff-nfs-storage-controller-setup.adoc.html
...
[NOTE]
Please note that disabling numeric ids requires user management, as described in the section link:hana-aff-nfs-install-prep-nfsv4.adoc.html[“SAP HANA installation preparations for NFSv4.”]

[NOTE]
The NFSv4 domain ID must be set to the same value on all Linux servers (`/etc/idmapd.conf`) and SVMs, as described in the section link:hana-aff-nfs-install-prep-nfsv4.adoc.html[“SAP HANA installation preparations for NFSv4.”]
