...
The Snapshots created by ONTAP Autonomous Ransomware Protection can't be used for forward recovery of the SAP HANA database. Therefore it is not recommended  to use it for SAP HANA data and log volumes and should be disabled for these volumes. Use SnapCenter based data protection with Tamperproof Snapshots instead for ransomware protection:
link:../backup/hana-sc-generic-scope.html[SAP HANA backup and recovery with SnapCenter].
ONTAP Autonomous Ransomware Protection might be beneficial for other volumes such as /hana/shared.

It is possible to disable the automatically activation of this feature during volume creation for an ONTAP Cluster, however it is recommended to keep the default setting and disable it for volumes where this functionality is not needed.
...
[NOTE]
If SAP Local Secure Store service (LSS) is used, the `/lss/shared/SID` directory, the default installation directory of LSS, is on the local disk. In a disaster recovery setup with disk-based replication, NetApp recommends creating a directory within the `SID_shared` volume for the `/lss/shared/SID` directory so that all file systems are on the central storage.
...
[NOTE]
If SAP Local Secure Store service (LSS) is used, the `/lss/shared/SID` directory must be accessible at all hosts. NetApp recommends creating a directory within the `SID_shared` volume for the `/lss/shared/SID` directory.
...
|Disable ransomware protection, except of the SID_shared volume
|vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled
...
[NOTE]
Please note that disabling numeric ids requires user management, as described in the section link:hana-aff-nfs-install-prep-nfsv4.html[“SAP HANA installation preparations for NFSv4.”]

[NOTE]
The NFSv4 domain ID must be set to the same value on all Linux servers (`/etc/idmapd.conf`) and SVMs, as described in the section link:hana-aff-nfs-install-prep-nfsv4.html[“SAP HANA installation preparations for NFSv4.”]
