...
[NOTE]
SAP HANA multiple-host systems require the `/hana/shared` file system  and the `/lss/shared` file systems, in case SAP Local Secure Store service(LSS) is used, connected to all hosts of a system. Usually this is a NFS share provided by by an NFS server. 
