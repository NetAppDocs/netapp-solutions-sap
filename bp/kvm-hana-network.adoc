...
In this chapter the SR-IOV functionality is enabled, the max amount of VFs (virtual interface) per physical port are set at the adapters firmware, and the amount of used VFs per physical port is set, four in this example.

[arabic]
. Run MFT (Mellanox Firmware Update Tool)
....
# mst start 
...
[NOTE] 
 If your configured already a bond using thess two ports the first command needs to be executed against to bond:
