...
It is recommended to use redundant ethernet connections. Therefore it is recommended to combine two physical ports as trunk/bond. If the physical ports are trunked also the virtual ports assigned to the VM need to be trunked, a bond needs to be created within the VM.
...
* KVM is installed
* SR_IOV is enabled in the servers BIOS
* PCI Paththrough has been enabled by adding “intel_iommu=on” and “iommu=pt” as option at the bootloader
* Latest MLNX_OFED drivers have been installed at KVM hosts and VM.
...
....
mlxconfig -d /dev/mst/mt4125_pciconf0 q |grep -e SRIOV_EN -e NUM_OF_VFS NUM_OF_VFS 8 
SRIOV_EN True(1)_
....

[arabic, start=4]
. If necessary, enable SR-IOV and set the amount of VFs 
....
mlxconfig -d /dev/mst/mt4125_pciconf0 set SRIOV_EN=1 NUM_OF_VFS=8 
....

[arabic, start=5]
. Reboot the server if the feature needed to be enabled.
...
If your configured already a bond using this two ports the first command needs to be executed against to bond:
...
In order to make it persistent a _systemd_ unit file /etc/ systemd/system/after.local needs to be created with the following content:
...
....
# cd /etc/init.d/
# chmod 777 after.local
....

== Assigning the virtual interfaces to the VM 

This chapter describes how to assign virtual interfaces to the VM using _virtmanager_

[arabic]
. Start _virt-manager +
image:kvm-hana-image2.png[width=601,height=385] +

. Open the desired VM__ +
...
Logon onto the VM and configure the two VFs as bond. Choose either mode 0 or mode 2. Do not use LACP as LACP can only be used on physical ports. The figure below shows a mode 2 configuration using YAST. +
 +
