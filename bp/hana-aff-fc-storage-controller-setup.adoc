---
sidebar: sidebar
permalink: bp/hana-aff-fc-storage-controller-setup.adoc.html
...
. Create initiator groups (igroups) with worldwide names (WWNs) of HANA servers as described in the section link:hana-fas-fc-storage-controller-setup.adoc.html#initiator-groups <<Initiator groups>>.
...
[NOTE]
The `sanlun` tool is part of the NetApp Host Utilities and must be installed on each SAP HANA host. More details can be found in section link:hana-aff-fc-host-setup.adoc.html[Host setup.]
