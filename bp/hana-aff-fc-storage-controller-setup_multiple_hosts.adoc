...
The `/hana/shared` volume must be accessible by all HANA hosts and is therefore exported by using NFS. Even though there are no specific performance KPIs for the `/hana/shared` and `/lss/shared` file systems, NetApp recommends using a 10Gb Ethernet connection.
...
[NOTE]
NetApp ASA systems do not support NFS as a protocol. NetApp recommends using an additional AFF or FAS system for the `/hana/shared` and `/lss/shared` file systems.

image:saphana_aff_fc_image17a.png["Figure showing input/output dialog or representing written content"]

For each SAP HANA host, a data volume and a log volume are created. The `/hana/shared` and `/lss/shared` volumes are used by all hosts of the SAP HANA system. The following table shows an example configuration for a 4+1 multiple-host SAP HANA system.
...
LUNs of an LVM volume group should be stored within a different aggregate and at a different controller. The following table shows an example for two LUNs per volume group for a 2+1 SAP HANA multiple host system.
...
|===
[NOTE]
The example below sets these options during volume creation
