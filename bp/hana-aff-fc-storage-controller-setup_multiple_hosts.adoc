...
[NOTE]
If SAP Local Secure Store service (LSS) is used, the `/lss/shared/SID` directory must be accessible at all hosts. NetApp recommends creating a volume for the `/lss/shared/SID` directory and mount it using NFS to all hosts.
...
vol create -volume FC5-lss_shared -aggregate aggr2_1 -size 512g -state online -policy default -snapshot-policy none -junction-path /FC5-lss_shared -encrypt false   -space-guarantee none -anti-ransomware-state disabled
....
