...
As described in the chapter link:hana-sc-generic-concepts.html#retention-management-of-backups-at-the-secondary-storage-snapvault["Retention mnanagement for secondary backups"],  retention management of data backups to an secondary backup storage is handled by ONTAP. SnapCenter periodically checks if ONTAP has deleted backups at the secondary backup storage by running a cleanup job with a weekly default schedule.
