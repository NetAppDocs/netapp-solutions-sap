...
[NOTE]
The SnapMirror label in the policy being created must match the label defined in the SnapCenter policy configuration. For details, refer to link:saphana-br-scs-snapcenter-initial-configuration.html#snapshot-policy[Policy for daily Snapshot backups with SnapVault replication].
...
. When resource configuration is finished, perform the configuration of resource protection as described in the section link:saphana-br-scs-snapcenter-resource-specific-configuration-for-sap-hana-database-backups.html#resource-protection[Resource protection configuration].
