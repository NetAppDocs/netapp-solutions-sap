...
With the recovery scope you can select a 'to most recent state', 'point in time' or a save point recovery without using log backups. If you select no recovery, SnapCenter only executes the restore operation and the recovery needs to be done manually as described link:hana-sc-generic-restore.html#manual-recovery-with-hana-studio["Manual recovery with HANA Studio"].
...
A single tenant restore and recovery operation with SnapCenter is very similar to the workflow described in the previous topic link:hana-sc-generic-restore.html#manual-recovery-with-hana-studio["Manual recovery with HANA Studio"].
