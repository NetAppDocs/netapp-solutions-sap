---
sidebar: sidebar
permalink: bp/hana-sc-generic-restore.html
...
image:hana-sc-generic-image-099.png[width=351,height=329]
...
With the recovery scope you can select a 'to most recent state', 'point in time' or a save point recovery without using log backups. If you select no recovery, SnapCenter only executes the restore operation and the recovery needs to be done manually as described in chapter Manual recovery with SnapCenter.
...
In HANA Studio select â€œBackup and Recoveryâ€ and â€œRecover System Databaseâ€.
...
Click on â€œFinishâ€ to start the restore operation.
...
A single tenant restore and recovery operation with SnapCenter is very similar to the workflow described in the previous chapter Manual recovery with HANA Studio.
