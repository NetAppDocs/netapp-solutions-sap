...
The initial SnapCenter configuration, as described in the previous topic link:hana-sc-generic-sc-initial-config.html["SnapCenter initial configuration"] must be done first, since credentials, storage systems and policies are required during the HANA database resource configuration. The figure below summarizes the steps and dependencies.
...
NOTE: The backup user must be created in the system database because all backup commands for the system and the tenant databases are executed via the system database.
