---
sidebar: sidebar
permalink: bp/hana-sc-generic-sc-resource-config.html
...
For ANF backup no specific preparation is required. As soon as the first backup with enabled ANF backup is executed an Azure backup vault with the name â€œsnapcenter-vaultâ€ is created by SnapCenter. This backup vault is then used by all following ANF backup operations executed by SnapCenter.
...
The HANA plug-in deployment is done by clicking the â€œAddâ€ button in the Hosts section of the SnapCenter UI.
