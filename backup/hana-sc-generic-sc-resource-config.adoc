...
The initial SnapCenter configuration, as described in the previous topic link:hana-sc-generic-initial-config.html["SnapCenter initial configuration"] must be done first, since credentials, storage systems and policies are required during the HANA database resource configuration. The figure below summarizes the steps and dependencies.
