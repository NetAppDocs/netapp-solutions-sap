...
Detailed information on SAP System Refresh operations using SnapCenter can be found at https://docs.netapp.com/us-en/netapp-solutions-sap/lifecycle/sc-copy-clone-introduction.html[Automating SAP HANA System Copy and Clone Operations with SnapCenter].
...
For SnapCenter releases < 6.2 there are limitations which need to be considered.
[arabic]
...
As mentioned in the “Pre-requisites” section, for SnapCenter releases < 6.2 the SnapCenter spl service at the HANA host must be restarted using the command “systemctl restart spl” to initiate proper cleanup.
