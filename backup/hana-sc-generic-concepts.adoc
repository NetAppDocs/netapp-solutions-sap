...
NetApp recommends using hdbpersdiag to execute HANA block consistency checks. Further details on the implementation are available in chapter link:hana-sc-generic-block-check.html["Block consistency checks with SnapCenter"].
...
NOTE: For this example configuration, hdbpersdiag is used for the block integrity check operation. More details can be found in chapter link:hana-sc-generic-block-check.html["Block consistency checks with SnapCenter"].
...
The cleanup job is scheduled once per week by default. This weekly schedule results in a delay with deleting backups in SnapCenter and SAP HANA Studio when compared with the backups that have already been deleted at the secondary storage. To avoid this inconsistency, customers can change the schedule to a higher frequency, for example, once per day. For details about how to adapt the schedule of the cleanup job or how to trigger a manual refresh, refer to the chapter link:hana-sc-generic-backup-operations.html#cleanup-of-secondary-backups["Cleanup of secondary backups"].
