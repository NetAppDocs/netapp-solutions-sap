---
sidebar: sidebar
permalink: backup/hana-sc-generic-sc-central-plugin.html
...
As discussed in chapter Deployment options for SnapCenter SAP HANA plug-in the HANA plug-in can be deployed outside of the HANA database to support a central plug-in configuration which is required SAP HANA multiple host systems or SAP HANA on IBM Power environments.
